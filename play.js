document.addEventListener("DOMContentLoaded", async () => {
  ...
  bacK.addEventListener('click', () => window.location.href = `next.html?id=${animeId}`);
  ...
  epsBtnContainer.style.maxHeight = "20rem"; 
      ...
      if (event.data && event.data.type === 'PLAYER_EVENT') {
        ...
        if (eventType === 'complete') {
          ...
          if (nextEp <= TotalEpisode) {
            window.location.href = `play.html?id=${animeId}&ep=${nextEp}`;
          }
        }
      }
    });
    ...
    PlaY.addEventListener('click', () => {
      window.location.href = `play.html?id=${animeId}&ep=1`;
    });
